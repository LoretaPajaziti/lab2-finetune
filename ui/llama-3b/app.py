...
# Base model + LoRA que ya has subido
BASE_MODEL = "unsloth/Llama-3.2-3B-Instruct"
# LORA_REPO = "lppapri/laurapp-lab2"
...
def extract_text(content):
    # Gradio v4: content can be str OR list[{"type": "text", "text": "..."}]
...
def extract_city_from_text(user_text: str, default_city: str = "Stockholm") -> str:
    ...
    """
    # Normalizamos a Title Case para ayudar al modelo cased
    text_for_ner = user_text.title()
    
    try:
        ents = ner_pipeline(user_text)
    except Exception:
        # Return default_city if NER model fails
        return default_city

    # Obtain location type entities
    locs = [e["word"] for e in ents if e.get("entity_group") in ("LOC", "ORG", "PER", "MISC")]
    # Si quieres ser más estricto, usa solo LOC
...
def get_external_context(user_text: str) -> str:
    ...
    # --- Tool 2: simple weather via wttr.in (optional) ---
    if any(word in user_lower for word in ["today", "weather", "clime", "forecast"]):  
        try:
            # You can hardcode your city or parse it from the message
            ...
            if cw and dw:
                ...
                # mapeo simple de weathercode a descripción
                mapping = {
...
def chat_fn(message, history):
    # history es una lista [(user, bot), ...], aquí la ignoramos para simplificar
    if history is None:
        history = []
    ...
    # 1) Mensaje de sistema opcional (recomendado)
    messages.append({
        "role": "system",
    ...
    # 2) Historial
    for m in history:
        # Por si acaso alguien devuelve algo raro, normalizamos un poco
    ...
    # 4) Construir el prompt usando la plantilla de chat del modelo
